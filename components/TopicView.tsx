...
const TopicView: React.FC = () => {
    ...
    // Convert slug back to display name (e.g. 'freelance-101' -> 'Freelance 101')
    ...
    return (
        <>
            <SEO title={topic} description={`Curated resources, playbooks, and tools related to ${topic}.`} path={`/topic/${topicSlug}`} />
            <div className="animate-fade-in pt-32 md:pt-48 pb-20 px-6 max-w-7xl mx-auto">
                ...
                {resources.length === 0 ? (
                    <div className="text-center py-20 bg-gray-50 rounded-sm border border-dashed border-gray-300">
                        ...
                        <Link to="/" className="mt-4 text-befinlit-navy font-bold hover:text-befinlit-gold transition-colors text-sm inline-block">
                            Go back Home
                        </Link>
                    </div>
                ) : (
